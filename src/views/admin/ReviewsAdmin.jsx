import { faLeftLong, faStar } from '@fortawesome/free-solid-svg-icons'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'
import React from 'react'
import { Container } from 'react-bootstrap'
import { NavLink } from 'react-router-dom'

export default function ReviewsAdmin() {
    return (
        <div className='reviews_admin mt-3'>
            <Container>
                <NavLink to="/adminSecret" className="back_to_admin"><FontAwesomeIcon icon={faLeftLong} />Back to admin dashboard</NavLink>
                <h1 className='text-center mb-5'>Reviews</h1>
                <div className="reviews">
                    <div className="review">
                        <div className="content">
                            <q>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Impedit, error et eos natus mollitia at accusantium facilis vitae harum aliquid id veniam quia a deserunt nam iusto dignissimos. Sit, quo.</q>
                        </div>
                        <div className="info">
                            <div className="name">Yossef Fekry</div>
                            <div className="stars">
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                            </div>
                        </div>
                    </div>
                    <div className="review">
                        <div className="content">
                            <q>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Impedit, error et eos natus mollitia at accusantium facilis vitae harum aliquid id veniam quia a deserunt nam iusto dignissimos. Sit, quo.</q>
                        </div>
                        <div className="info">
                            <div className="name">Yossef Fekry</div>
                            <div className="stars">
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                            </div>
                        </div>
                    </div>
                    <div className="review">
                        <div className="content">
                            <q>Lorem ipsum, dolor sit amet consectet Lorem ipsum dolor sit amet consectetur adipisicing elit. Laudantium reiciendis exercitationem eveniet excepturi, animi perferendis qui natus. Sed labore eveniet ullam excepturi voluptatem quaerat aliquid natus rem, vitae, unde doloremque!Loremspfjweodmal;cmsnfjindf ur adipisicing elit. Impedit, error et eos natus mollitia at accusantium facilis vitae harum aliquid id veniam quia a deserunt nam iusto dignissimos. Sit, quo.</q>
                        </div>
                        <div className="info">
                            <div className="name">Yossef Fekry</div>
                            <div className="stars">
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                            </div>
                        </div>
                    </div>
                    <div className="review">
                        <div className="content">
                            <q>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Impedit, error et eos natus mollitia at accusantium facilis vitae harum aliquid id veniam quia a deserunt nam iusto dignissimos. Sit, quo.</q>
                        </div>
                        <div className="info">
                            <div className="name">Yossef Fekry</div>
                            <div className="stars">
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                            </div>
                        </div>
                    </div>
                    <div className="review">
                        <div className="content">
                            <q>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Impedit, error et eos natus mollitia at accusantium facilis vitae harum aliquid id veniam quia a deserunt nam iusto dignissimos. Sit, quo.</q>
                        </div>
                        <div className="info">
                            <div className="name">Yossef Fekry</div>
                            <div className="stars">
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                            </div>
                        </div>
                    </div>
                    <div className="review">
                        <div className="content">
                            <q>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Impedit, error et eos natus mollitia at accusantium facilis vitae harum aliquid id veniam quia a deserunt nam iusto dignissimos. Sit, quo.</q>
                        </div>
                        <div className="info">
                            <div className="name">Yossef Fekry</div>
                            <div className="stars">
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                            </div>
                        </div>
                    </div>
                    <div className="review">
                        <div className="content">
                            <q>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Impedit, error et eos natus mollitia at accusantium facilis vitae harum aliquid id veniam quia a deserunt nam iusto dignissimos. Sit, quo.</q>
                        </div>
                        <div className="info">
                            <div className="name">Yossef Fekry</div>
                            <div className="stars">
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                            </div>
                        </div>
                    </div>
                    <div className="review">
                        <div className="content">
                            <q>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Impedit, error et eos natus mollitia at accusantium facilis vitae harum aliquid id veniam quia a deserunt nam iusto dignissimos. Sit, quo.</q>
                        </div>
                        <div className="info">
                            <div className="name">Yossef Fekry</div>
                            <div className="stars">
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                            </div>
                        </div>
                    </div>
                    <div className="review">
                        <div className="content">
                            <q>Lorem ipsum, dolor sit amet consectetur adipisicing elit. Impedit, error et eos natus mollitia at accusantium facilis vitae harum aliquid id veniam quia a deserunt nam iusto dignissimos. Sit, quo.</q>
                        </div>
                        <div className="info">
                            <div className="name">Yossef Fekry</div>
                            <div className="stars">
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                                <FontAwesomeIcon icon={faStar} />
                            </div>
                        </div>
                    </div>
                </div>
            </Container>
        </div>
    )
}
